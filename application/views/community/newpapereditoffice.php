    ...
    <div class="content-wrapper">
        <div class="row">
            <div class="col-12 grid-margin stretch-card">
                <div class="card">
                    <div class="card-body">
                        <form id="updateofiiceinfo" method="POST" enctype="multipart/form-data" >
                            <div class="form-group">
                                <input type="text" name="upofid" value="<?php echo $rw ['id'] ; ?>" />
                                <input type="text" name="upimgup" multiple="true" value="<?php echo $rw ['npofficeimage'];?>" />
                            </div>
                            <div class="form-group">
                                <label for="exampleInputName1">office Name</label>
                                <input type="text" class="form-control" id="exampleInputName1" placeholder="Name" name="upofficename" required="required" value="<?php echo $rw ['npofficename']; ?>" />
                            </div>

                            <div class="form-group">
                                <label>Office Description</label>
                                <textarea class="form-control" name="upofficedesc" id="input-4" required="required"><?php echo $rw['npdec']; ?></textarea>
                            </div>

                            <?php 
              $desired= json_decode($rw['npofficmobile']);   
              if(empty($desired)){
                  $desired=[];
                 }            
               ?>
                ...
                if(!empty($desired)){
                  for ($i=0; $i < count($desired); $i++) {
                    ?>
                            ...
                            <?php 
              $Landline= json_decode($rw['npofficelandline']);   
              if(empty($Landline)){
                  $Landline=[];
                 }            
               ?>
                ...
                if(!empty($Landline)){
                  for ($i=0; $i < count($Landline); $i++) {
                    ?>
                            ...
                            <?php 
              $Email= json_decode($rw['npofficehemail']);   
              if(empty($Email)){
                  $Email=[];
                 }            
               ?>
                  ...
                  for ($i=0; $i < count($Email); $i++) {
                    ?>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="upofficefacebook" placeholder="Facebook Page Link" class="form-control" value="<?php echo $rw['npofficefacebook']; ?>" />
                            </div>
                            <div class="form-group">
                                <label> Instagram Page Link: <span>*</span></label>
                                <input type="text" name="upofficeinstagram" placeholder="Instagram Page Link" class="form-control" value="<?php echo $rw['npofficeinstagram']; ?>" />
                            </div>
                            <div class="row">
                                <div class="col-md-4">
                                    <img
                                        class="proimg mt-2"
                                        src="<?php $imgs = $rw['npofficeimage']; 
                    if ($imgs) {echo base_url().''.$rw['npofficeimage'];} else {echo base_url().'uploads/origin/def-img.png';}?>"
                            ...
                            <div>
                                <input type="submit" id="submit" value="Submit" name="submit" class="btn btn-primary col-md-2" />
                            </div>
                        </form>
                    </div>
                </div>
...
$('#updateofiiceinfo').submit(function(e){
            ...
            $.ajax({
             url:'<?php echo base_url();?>Newspaper/newupdateofiiceinfos',
             type:"POST",
            dataType: "json",
